...
@Composable
fun ViewCheckOuts(navController: NavController) {
    ...
    val todayCheckOuts = viewModel.getTodayCheckOuts(roomsWithCustomers.value)
...
@Composable
fun CheckOuts( viewModel: ViewBookingViewModel) {
    val roomsWithCustomers = viewModel.roomsWithCustomers.observeAsState(initial = emptyList())
    val todayCheckOuts = viewModel.getTodayCheckOuts(roomsWithCustomers.value)
    ...
    LazyColumn(
        ...
        items(todayCheckOuts) { checkOut ->
            Row(
                ...
                Text(
                    text = checkOut.room.roomNo,
                    fontSize = 16.sp,
                    fontWeight = FontWeight.Normal,
                    modifier = Modifier.weight(1f)
                )
                Text(
                    text = "${checkOut.room.roomPrice}",
                    fontSize = 16.sp,
                    fontWeight = FontWeight.Normal,
                    modifier = Modifier.weight(1f)
                )
                if (checkOut.customers.isNotEmpty()) {
                    Text(
                        text = checkOut.customers[0].name,
                        fontSize = 16.sp,
                        fontWeight = FontWeight.Normal,
                        modifier = Modifier.weight(1f)
                    )
        ...
        item {
            Spacer(modifier = Modifier.size(8.dp))
            Text(
                text = "Total Check Ins: ${todayCheckOuts.size}",
                fontSize = 16.sp,
                fontWeight = FontWeight.Bold
            )
            Spacer(modifier = Modifier.size(8.dp))
...
@Composable
fun ViewCheckOutPreview() {
    DiamondGuestHouseTheme {
        ViewCheckOuts( navController = rememberNavController())
    }
}